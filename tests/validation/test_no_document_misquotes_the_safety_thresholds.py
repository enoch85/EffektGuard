...
def test_no_document_teaches_the_flow_temperature_model_that_was_removed(path):
    ...
    `utils/emitter.py`. Against NIBE's own published curve 9, EN 442 lands 0.20 °C away where a
    straight line is out by 2.37 °C.

    A document may explain what Kühne WAS and why it went - `docs/research/02_emitter_law.md` does,
    and that is the point of it. A document may not still be teaching it.
    """
    claims = _paragraphs_that_assert(path)

    assert "Kühne" not in claims and "Kuhne" not in claims, (
        f"{path.relative_to(ROOT)} still teaches André Kühne's flow-temperature formula. It "
        f"appears ZERO times in the codebase - it was replaced by the EN 442 emitter law. A reader "
        f"following this document builds the model this project deliberately removed. "
