...
from custom_components.effektguard.number import EffektGuardNumber
from custom_components.effektguard.select import EffektGuardSelect
...
class TestSelectEntityConfigStorage:
    """Test that select entities update entry.options, not entry.data."""

    @pytest.mark.asyncio
    async def test_select_reads_from_options_first(self, mock_coordinator, mock_config_entry):
        """Verify select entity reads from options (not data)."""
        from custom_components.effektguard.select import SELECTS

        # Create select entity for optimization mode
        description = SELECTS[0]  # optimization_mode
        select = EffektGuardSelect(mock_coordinator, mock_config_entry, description)

        # Should read from options
        current = select.current_option
        assert current == OPTIMIZATION_MODE_BALANCED
        assert current == mock_config_entry.options[CONF_OPTIMIZATION_MODE]

    @pytest.mark.asyncio
    async def test_select_updates_options_not_data(self, mock_coordinator, mock_config_entry):
        """Verify select entity updates entry.options, not entry.data."""
        from custom_components.effektguard.select import SELECTS
        ...
        # Mock async_write_ha_state to avoid HA internals
        select.async_write_ha_state = Mock()

        # Change option
        await select.async_select_option(OPTIMIZATION_MODE_SAVINGS)

        # Verify async_update_entry was called with options
        mock_coordinator.hass.config_entries.async_update_entry.assert_called_once()
        call_args = mock_coordinator.hass.config_entries.async_update_entry.call_args

        # Check that options were updated
        assert call_args[0][0] == mock_config_entry  # First arg is entry
        assert "options" in call_args[1]  # Keyword arg
        assert call_args[1]["options"][CONF_OPTIMIZATION_MODE] == OPTIMIZATION_MODE_SAVINGS

        # Verify coordinator refresh was requested
        mock_coordinator.async_request_refresh.assert_called_once()

    @pytest.mark.asyncio
    async def test_select_writes_state_immediately(self, mock_coordinator, mock_config_entry):
        """Verify select entity calls async_write_ha_state() for immediate frontend update."""
        from custom_components.effektguard.select import SELECTS

        description = SELECTS[0]
        select = EffektGuardSelect(mock_coordinator, mock_config_entry, description)
        select.hass = mock_coordinator.hass
        select.async_write_ha_state = Mock()  # Mock the method

        await select.async_select_option(OPTIMIZATION_MODE_SAVINGS)

        # Should write state immediately (before coordinator refresh completes)
        select.async_write_ha_state.assert_called_once()


class TestNumberEntityConfigStorage:
    """Test that number entities update entry.options, not entry.data."""

    @pytest.mark.asyncio
    async def test_number_reads_from_options_first(self, mock_coordinator, mock_config_entry):
        """Verify number entity reads from options (not data)."""
        from custom_components.effektguard.number import NUMBERS

        # Create number entity for target temperature
        description = NUMBERS[0]  # target_temperature
        number = EffektGuardNumber(mock_coordinator, mock_config_entry, description)

        # Should read from options
        value = number.native_value
        assert value == 21.0
        assert value == mock_config_entry.options[CONF_TARGET_INDOOR_TEMP]

    @pytest.mark.asyncio
    async def test_number_updates_options_not_data(self, mock_coordinator, mock_config_entry):
        """Verify number entity updates entry.options, not entry.data."""
        from custom_components.effektguard.number import NUMBERS
        ...
        # Change value
        await number.async_set_native_value(23.0)
        ...
        assert call_args[1]["options"][CONF_TARGET_INDOOR_TEMP] == 23.0

        # Verify coordinator refresh was requested
        mock_coordinator.async_request_refresh.assert_called_once()

    @pytest.mark.asyncio
    async def test_number_writes_state_immediately(self, mock_coordinator, mock_config_entry):
        """Verify number entity calls async_write_ha_state() for immediate frontend update."""
        from custom_components.effektguard.number import NUMBERS
        ...
        # Should write state immediately
...
class TestCoordinatorConfigUpdate:
    ...
    async def test_updates_config_dict_values(self, mock_coordinator):
        ...
        new_options = {
            CONF_OPTIMIZATION_MODE: OPTIMIZATION_MODE_SAVINGS,
            CONF_CONTROL_PRIORITY: CONTROL_PRIORITY_SAVINGS,
            CONF_PEAK_PROTECTION_MARGIN: 1.0,
        }
        await coordinator.async_update_config(new_options)

        # Should update config dict
        assert coordinator.engine.config[CONF_OPTIMIZATION_MODE] == OPTIMIZATION_MODE_SAVINGS
        assert coordinator.engine.config[CONF_CONTROL_PRIORITY] == CONTROL_PRIORITY_SAVINGS
        assert coordinator.engine.config[CONF_PEAK_PROTECTION_MARGIN] == 1.0

    @pytest.mark.asyncio
    async def test_triggers_coordinator_refresh(self, mock_coordinator):
        """Verify async_update_config triggers immediate coordinator refresh."""
        from custom_components.effektguard.coordinator import EffektGuardCoordinator
...
class TestEndToEndConfigReload:
    """End-to-end integration tests for config reload flow."""

    @pytest.mark.asyncio
    async def test_temperature_change_full_flow(
        self, mock_hass, mock_config_entry, mock_coordinator
    ):
        """Test complete flow: user changes temperature → engine uses new value."""
        from custom_components.effektguard.number import NUMBERS
        from custom_components.effektguard.coordinator import EffektGuardCoordinator
        ...
        # Create a wrapped version that tracks calls
        original_update = EffektGuardCoordinator.async_update_config
        update_called = []

        async def tracked_update(self, options):
            update_called.append(options)
            return await original_update(self, options)

        mock_coordinator.async_update_config = tracked_update.__get__(
            mock_coordinator, EffektGuardCoordinator
        )

        # Initial state
        assert mock_coordinator.engine.target_temp == 21.0

        # Step 1: User changes temperature via number entity
        description = NUMBERS[0]  # target_temperature
        number = EffektGuardNumber(mock_coordinator, mock_config_entry, description)
        number.hass = mock_hass
        number.async_write_ha_state = Mock()  # Mock to avoid HA internals

        await number.async_set_native_value(23.0)

        # Step 2: Config entry updated
        assert mock_hass.config_entries.async_update_entry.called

        # Step 3: Update listener triggered (simulate)
        mock_config_entry.options = {CONF_TARGET_INDOOR_TEMP: 23.0}
        await async_reload_entry(mock_hass, mock_config_entry)

        # Step 4: Coordinator updated
        assert len(update_called) > 0, "async_update_config was not called"

        # Step 5: Decision engine has new value
        assert mock_coordinator.engine.target_temp == 23.0

        # Step 6: Coordinator refresh requested
        assert mock_coordinator.async_request_refresh.called

    @pytest.mark.asyncio
    async def test_optimization_mode_change_full_flow(
        self, mock_hass, mock_config_entry, mock_coordinator
    ):
        """Test complete flow: user changes optimization mode → config dict updated."""
        from custom_components.effektguard.select import SELECTS
        from custom_components.effektguard.coordinator import EffektGuardCoordinator

        # Setup
        mock_hass.data[DOMAIN][mock_config_entry.entry_id] = mock_coordinator
        mock_coordinator.async_update_config = EffektGuardCoordinator.async_update_config.__get__(
            mock_coordinator, EffektGuardCoordinator
        )

        # Initial state
        assert mock_coordinator.engine.config[CONF_OPTIMIZATION_MODE] == OPTIMIZATION_MODE_BALANCED

        # User changes optimization mode via select entity
        description = SELECTS[0]  # optimization_mode
        select = EffektGuardSelect(mock_coordinator, mock_config_entry, description)
        select.hass = mock_hass
        select.async_write_ha_state = Mock()  # Mock to avoid HA internals
        ...
        # Config dict updated
        assert mock_coordinator.engine.config[CONF_OPTIMIZATION_MODE] == OPTIMIZATION_MODE_SAVINGS


class TestRuntimeOptionsCompleteness:
    """Verify all runtime options are included in the hot-reload list."""

    def test_all_select_options_are_runtime(self):
        """Verify all select entity config keys are in runtime_options set."""
        from custom_components.effektguard.select import SELECTS

        # Extract config keys from select entities
        select_config_keys = set()
        for desc in SELECTS:
            if desc.config_key:
                select_config_keys.add(desc.config_key)

        # These should all be in runtime_options (from __init__.py)
        runtime_options = {
            "target_indoor_temp",
            "tolerance",
            "optimization_mode",
        ...
        # All select config keys should be runtime options
        assert select_config_keys.issubset(
            runtime_options
        ), f"Select keys not in runtime_options: {select_config_keys - runtime_options}"

    def test_all_number_options_are_runtime(self):
        """Verify all number entity config keys are in runtime_options set."""
        from custom_components.effektguard.number import NUMBERS

        # Extract config keys from number entities
        number_config_keys = set()
        for desc in NUMBERS:
            if desc.config_key:
                number_config_keys.add(desc.config_key)
        ...
        # All number config keys should be runtime options
        assert number_config_keys.issubset(
            runtime_options
        ), f"Number keys not in runtime_options: {number_config_keys - runtime_options}"
...
class TestCompleteChainValidation:
    ...
    @pytest.mark.asyncio
    async def test_chain_user_to_decision_engine(
        self, mock_hass, mock_config_entry, mock_coordinator
    ):
        """Validate: User UI change → Entity → Options → Coordinator → Engine → Decision."""
        from custom_components.effektguard.number import NUMBERS
        from custom_components.effektguard.coordinator import EffektGuardCoordinator
        ...
        assert mock_config_entry.options[CONF_TARGET_INDOOR_TEMP] == 21.0
        assert mock_coordinator.engine.target_temp == 21.0

        # Step 1: User changes temperature in UI
        description = NUMBERS[0]  # target_temperature
        number = EffektGuardNumber(mock_coordinator, mock_config_entry, description)
        number.hass = mock_hass
        number.async_write_ha_state = Mock()

        # Step 2: Entity updates options
        await number.async_set_native_value(23.0)

        # Verify options updated
        call_args = mock_hass.config_entries.async_update_entry.call_args
        updated_options = call_args[1]["options"]
        assert updated_options[CONF_TARGET_INDOOR_TEMP] == 23.0

        # Step 3: Simulate update listener calling async_update_config
        mock_config_entry.options = updated_options
        await mock_coordinator.async_update_config(mock_config_entry.options)

        # Step 4: Verify decision engine cache updated
        assert mock_coordinator.engine.target_temp == 23.0

        # Step 5: Verify coordinator refresh requested
        assert mock_coordinator.async_request_refresh.called

        # CHAIN VERIFIED: User → Entity → Options → Coordinator → Engine ✓
    ...
    @pytest.mark.asyncio
    async def test_frontend_immediate_feedback(self, mock_coordinator, mock_config_entry):
        """Validate: Frontend receives immediate feedback via async_write_ha_state."""
        from custom_components.effektguard.select import SELECTS
        ...
        # Mock async_write_ha_state to avoid HA internals
        write_called = []
        select.async_write_ha_state = lambda: write_called.append(True)

        # Change option
        await select.async_select_option(OPTIMIZATION_MODE_SAVINGS)

        # Should call async_write_ha_state immediately
        assert len(write_called) == 1, "async_write_ha_state not called"

        # CHAIN VERIFIED: Entity change → Immediate frontend update ✓
