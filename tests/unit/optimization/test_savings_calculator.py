...
class TestMonthlySavingsEstimation:
    ...
    def test_without_a_measured_baseline_there_is_no_effect_saving(self):
        ...
        assert estimate.effect_baseline_measured is False
        assert estimate.effect_savings == 0.0, (
            f"With no unoptimised baseline ever observed, the calculator reported "
            f"{estimate.effect_savings:.0f} SEK of effect-tariff savings. That number is "
            f"0.176 x peak x tariff by construction - computed from the peak itself - so a worse "
            f"peak reports a bigger saving, and it can never read zero."
        )

    def test_a_bigger_peak_can_never_report_a_bigger_saving(self):
        """The property that made the old sensor unfalsifiable, pinned so it cannot come back."""
        calc = SavingsCalculator()

        savings = [
            calc.estimate_monthly_savings(current_peak_kw=peak).effect_savings
            for peak in (3.0, 6.0, 9.0)
        ]

        assert savings == [0.0, 0.0, 0.0], (
            f"Effect savings rose with the peak: {savings} for peaks of 3, 6 and 9 kW. The saving "
            f"was being computed FROM the peak, so a house doing worse looked like it was saving "
            f"more."
        )

    def test_a_measured_baseline_produces_a_real_saving(self):
        """The mechanism must still work once a baseline actually exists."""
        calc = SavingsCalculator()
        calc.update_baseline_peak(10.0)  # observed while optimization was switched OFF

        estimate = calc.estimate_monthly_savings(
            current_peak_kw=8.0,
            baseline_peak_kw=calc.baseline_monthly_peak,
            average_spot_savings_per_day=0.0,
        )
    ...
    def test_a_measured_baseline_that_is_worse_reports_no_saving_not_a_negative_one(self):
        """The optimiser did worse than the baseline. That is zero saving, never a negative one."""
        calc = SavingsCalculator()
        calc.update_baseline_peak(6.0)
