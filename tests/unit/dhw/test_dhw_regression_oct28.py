...
def create_oct28_price_periods():
    ...
    prices = [
        ...
        8.6, 8.6, 8.7, 8.9,  # Q16 is 8.6 (cheapest)
        # 05:00-06:00 (Q20-Q23)
        9.2, 9.5, 9.8, 10.1,
        # 06:00-07:00 (Q24-Q27)
        10.5, 11.2, 12.5, 13.8,
...
def test_oct28_at_04_00_exactly():
    ...
    assert decision.should_heat is True, (
        f"Must heat at 04:00 (optimal window start). "
        f"Reason: {decision.priority_reason}"
    )
    ...
    assert any(reason in decision.priority_reason for reason in valid_reasons), (
        f"Should use optimal window or emergency heating logic. "
        f"Got: {decision.priority_reason}"
    )
    
    # Should target reasonable temp (either pre-heat or user target)
    assert 50.0 <= decision.target_temp <= 60.0, (
