...
fig.suptitle(
    "EffektGuard Price Optimization: Current Bug vs Expected Behavior\n(Based on Dec 5-6, 2025 real prices)",
    fontsize=14,
    fontweight="bold",
)
...
for i in range(len(all_prices) - 1):
    ax1.fill_between(
        [hours_from_now[i], hours_from_now[i + 1]],
...
ax1.legend(
    handles=[peak_patch, expensive_patch, normal_patch, cheap_patch], loc="upper right", fontsize=9
)
...
ax3.set_title(
    "EXPECTED Behavior (Fixed): Reduce now, pre-heat during cheap tomorrow",
    fontsize=11,
    color="green",
)
...
plt.tight_layout()
plt.savefig(
    str(Path(__file__).resolve().parents[1] / "docs" / "dev" / "price_optimization_comparison.png"),
    dpi=150,
    bbox_inches="tight",
)
plt.show()
