...
3. **Safety-first approach** - Heat pump health and comfort over cost savings
4. **No backward compatibility** - Rename directly, update all callers, no aliases
5. **Test after every change** - Clear `__pycache__`, run tests, verify heat pump behavior
...
8. **Ask before acting** - When uncertain about NIBE behavior, clarify first
9. **No analysis/summary files in git** - All `*ANALYSIS*.md`, `*SUMMARY*.md` files stay untracked
10. **Follow Home Assistant best practices** - Use official patterns, coordinator, config flow
11. **Read NIBE documentation** - Never guess heat pump behavior, verify with research docs
12. **Use Black formatting** - All Python code must be formatted with Black (line length 100)
...
grep -r "= 6.0" custom_components/effektguard/  # UFH horizons
